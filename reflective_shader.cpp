    ...
    const vec3& normal,int recursion_depth) const
{
	...
	color = vec3(0,0,0);//shader->Shade_Surface(ray,intersection_point,normal,recursion_depth);
    }
    else {
    ...
    if(debug_pixel) { std::cout << "(" << recursion_depth << "/" << world.recursion_depth_limit << ") reflective color: " << color << std::endl; }
    return color;
}
